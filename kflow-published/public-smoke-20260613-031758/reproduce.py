...
ASSET_URL = 'https://github.com/kyuhank/AnalysisFlowDemo/releases/download/kflow-flow-da59c4949540-public-smoke-20260613-031758/kflow-flow-da59c4949540-public-smoke-20260613-031758-outputs.tar.gz'
ASSET_API_URL = 'https://api.github.com/repos/kyuhank/AnalysisFlowDemo/releases/assets/446278073'
ASSET_NAME = 'kflow-flow-da59c4949540-public-smoke-20260613-031758-outputs.tar.gz'
ASSET_SHA256 = '55764968cdd0c3287c772e5ec2424b8704a5bfe73aa6e08231a1662a127bd55e'
...
def rerun_local():
    ...
    nodes_by_id = {str(node.get("id") or ""): node for node in nodes if isinstance(node, dict)}
    input_slugs_by_id = {
        str(node.get("id") or ""): [
            str(link.get("slug") or "")
            for link in node.get("inputs", [])
            if isinstance(link, dict) and str(link.get("slug") or "")
        ]
        for node in nodes
    ...
    for node in manifest.get("nodes", []):
        ...
        for parent_slug in input_slugs_by_id.get(str(node.get("id") or ""), []):
            parent_output = root / "outputs" / parent_slug
            if not parent_output.exists():
                parent_output = ROOT / "nodes" / parent_slug / "outputs"
            if parent_output.exists():
                shutil.copytree(parent_output, input_dir / parent_slug, dirs_exist_ok=True)
        ...
        if source_outputs.exists():
            shutil.copytree(source_outputs, output_dir, dirs_exist_ok=True)
